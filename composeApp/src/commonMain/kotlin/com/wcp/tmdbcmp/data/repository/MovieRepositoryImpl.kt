...
class MovieRepositoryImpl(
    ...
    override suspend fun getTrendingTodayMovies(nextPage: Int): Result<MoviesModel> = runCatching {
        movieApi.getTrendingTodayMovies(nextPage).toMoviesModel()
    }

    override suspend fun getPopularMovies(nextPage: Int): Result<MoviesModel> = runCatching {
        movieApi.getPopularMovies(nextPage).toMoviesModel()
    }

    override suspend fun getUpcomingMovies(nextPage: Int): Result<MoviesModel> = runCatching {
        movieApi.getUpcomingMovies(nextPage).toMoviesModel()
    }

    override suspend fun getTopRatedMovies(nextPage: Int): Result<MoviesModel> = runCatching {
        movieApi.getTopRatedMovies(nextPage).toMoviesModel()
    }

    override suspend fun getNowPlayingMovies(nextPage: Int): Result<MoviesModel> = runCatching {
        movieApi.getNowPlayingMovies(nextPage).toMoviesModel()
    }

    override suspend fun getMovieGenres(): Result<GenresModel> = runCatching {
        movieApi.getMovieGenres().toGenresModel()
    }

    override suspend fun getMovieDetails(movieId: Int): Result<MovieDetailsModel> = runCatching {
    ...
    override suspend fun searchMovie(query: String): Result<List<MovieModel>> = runCatching {
    ...
    override suspend fun toggleFavorite(model: FavoriteMovieModel): Result<Unit> = runCatching {
        if (model.favorite) {
